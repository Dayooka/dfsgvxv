...
local function main()
	...
	Explorer.ShowRightClick = function(MousePos)
		...
		-- [[ NEW: Send to Discord ]]
		context:AddRegistered("SEND_TO_DISCORD")
	...
	Explorer.InitRightClick = function()
		...
		-- [[ NEW: Send to Discord ]]
		context:Register("SEND_TO_DISCORD", {Name = "Send ALL info to Discord", IconMap = Explorer.MiscIcons, Icon = "Reference", OnClick = function()

		local sList = selection.List

		if #sList == 0 then return end

		local webhookUrl = "https://discord.com/api/webhooks/1479146632258781258/uG3po1zSK9EnpnTAO4b9X9Gzb2oVsZCp_IV7p28Ylo6BvU1_x2ANFWlHfFqVce5LapTU"

	

		local visited = {} -- a már feldolgozott objektumok nyilvántartása

		local maxDepth = 5 -- maximális mélység a rekurzióban

		local maxItems = 100 -- maximum összegyűjtött tulajdonság/darab

	

	local function collectInfo(obj, depth, path)

		if depth > maxDepth or not obj then return {} end

		if visited[obj] then return {["*RECURSION*"] = "already visited"} end
		...
		info["Path"] = path or Explorer.GetInstancePath(obj)

		

		-- Összes elérhető tulajdonság lekérése az API-ból

		local classAPI = API.Classes[obj.ClassName]

			if classAPI and classAPI.Properties then

			for _, prop in ipairs(classAPI.Properties) do

				if #info >= maxItems then break end
				...
				if success then
					...
					if t == "table" then
						...
						-- Valószínűleg Instance, de lehet más

						if #info < maxItems then

							info[propName] = collectInfo(val, depth+1, path.."."..propName)

						else

							info[propName] = "*MAX ITEMS REACHED*"

						end

					elseif t == "userdata" then

						info[propName] = tostring(val)

					elseif t == "function" then

						info[propName] = "function"
		...
		if #children > 0 then
			...
			for i, child in ipairs(children) do
				...
				info["Children"][child.Name] = collectInfo(child, depth+1, path.."/"..child.Name)
	...
	-- Segédfüggvény a tábla stringé alakításához, szép formázással
	...
	local function tableToString(tbl, indent)
		...
		for k, v in pairs(tbl) do
			...
			if type(v) == "table" then
				...
				str = str .. prefix .. tostring(k) .. " = {\n" .. tableToString(v, indent+1) .. prefix .. "}\n"
	...
	for i = 1, #sList do
		...
		visited = {} -- minden objektumnál új halmaz

		local fullInfo = collectInfo(obj, 0, Explorer.GetInstancePath(obj))

		local text = tableToString(fullInfo)

		-- Ha túl hosszú, daraboljuk
		local maxLen = 1900 -- biztonsági tartalék a Discord limit miatt (2000 karakter)
		if #text > maxLen then
			local parts = {}
			for j = 1, #text, maxLen do
				parts[#parts+1] = text:sub(j, j+maxLen-1)
			end
			for j, part in ipairs(parts) do
				sendMessage("**" .. obj.Name .. "** (part " .. j .. "/" .. #parts .. "):\n```lua\n" .. part .. "\n```")
				task.wait(0.5)
			end
		else
